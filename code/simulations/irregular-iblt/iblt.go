...
var regularThree = degreeDistribution{
	Name:  "regular_3",
	Terms: []degreeTerm{{Degree: 3, Probability: 1}},
}
...
func (d degreeDistribution) secondMoment() float64 {
	second := 0.0
	for _, term := range d.Terms {
		second += term.Probability * float64(term.Degree*term.Degree)
	}
	return second
}

func (d degreeDistribution) variance() float64 {
	mean := d.mean()
	return d.secondMoment() - mean*mean
}
...
// meanDegreeGamma is the tempting but incorrect plug-in that treats the
// random degree as if every item had fixed degree E[D].
func (d degreeDistribution) meanDegreeGamma(cells int) float64 {
	mean := d.mean()
	return mean - mean*mean/float64(cells)
}
